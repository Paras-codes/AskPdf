...
async def ask_question(req: QueryRequest):
    ...
    result = await qa_chain({"query": req.question})
...
async def delete_docs(req: DeleteRequest):
    vectordb = get_vectordb()
    await vectordb.delete(ids=req.ids)
...
async def delete_all_docs():
    vectordb = get_vectordb()
    all_ids = await vectordb.get()["ids"]
    if all_ids:
        await vectordb.delete(ids=all_ids)
